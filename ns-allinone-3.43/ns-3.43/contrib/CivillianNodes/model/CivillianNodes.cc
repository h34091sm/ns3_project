...
// Configure the nodes according to input coordinates and grid size. 
void CivillianNodes::add_nodes(std::vector<ns3::Vector3D>& node_coordinates, int gridSize)
{
    ...
    civillian_mobility.SetPositionAllocator(list_position_alloc);
    ...
    // civillians use a random waypoint mobiliy model 
    civillian_mobility.SetMobilityModel("ns3::RandomWaypointMobilityModel",
                                     "Speed", StringValue("ns3::ConstantRandomVariable[Constant=10.0]"),
                                      // "Pause", StringValue("ns3::ConstantRandomVariable[Constant=50.0]"),
                                     "PositionAllocator", PointerValue(randomAlloc));    
    civillian_mobility.Install(container);
